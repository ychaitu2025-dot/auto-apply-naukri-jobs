# naukri_auto_apply/app_tkinter.py
...
try:
    ...
    from core.naukri_login import login_to_naukri, update_naukri_credentials, validate_naukri_credentials
...
class NaukriAutoBotApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Naukri Auto Apply Bot")
        ...
        if os.path.exists(self.config_file):
            try:
                with open(self.config_file, 'r') as f:
                    ...
                    self.experience = config.get('experience', self.experience)
                    self.workplace_type = config.get('workplace_type', self.workplace_type)
    ...
    def save_config(self):
        ...
        try:
            config = {
                ...
                'workplace_type': self.workplace_var.get()
            }
    ...
    def setup_main_tab(self):
        ...
        advanced_frame = ttk.LabelFrame(self.main_tab, text="Advanced Filters:")
        advanced_frame.pack(fill="x", padx=10, pady=10)
        
        # Location
        location_frame = ttk.Frame(advanced_frame)
        location_frame.pack(fill="x", padx=10, pady=5)
        ttk.Label(location_frame, text="Location:", width=15).pack(side="left")
        ...
        # Experience Dropdown
        exp_frame = ttk.Frame(advanced_frame)
        exp_frame.pack(fill="x", padx=10, pady=5)
        ttk.Label(exp_frame, text="Experience:", width=15).pack(side="left")
        self.experience_var = tk.StringVar(value=self.experience)
        exp_options = ["0 years (Fresher)", "1 year", "2 years", "3 years", "4 years", "5 years", "6 years", "7 years", "8+ years"]
        self.experience_dropdown = ttk.Combobox(exp_frame, textvariable=self.experience_var, values=exp_options, state="readonly", width=27)
        self.experience_dropdown.pack(side="left")
        
        # Workplace Type Dropdown
        wp_frame = ttk.Frame(advanced_frame)
        wp_frame.pack(fill="x", padx=10, pady=5)
        ttk.Label(wp_frame, text="Workplace Type:", width=15).pack(side="left")
        self.workplace_var = tk.StringVar(value=self.workplace_type)
        wp_options = ["Any", "Remote", "Hybrid", "In Office"]
        self.workplace_dropdown = ttk.Combobox(wp_frame, textvariable=self.workplace_var, values=wp_options, state="readonly", width=27)
        self.workplace_dropdown.pack(side="left")
    ...
    def setup_settings_tab(self):
        ...
        # Login settings
        login_frame = ttk.LabelFrame(self.settings_tab, text="Naukri Login")
...
1. Enter your Naukri.com login credentials in the Settings tab and test them
...
- not_applied_jobs.xlsx - List of jobs that couldn't be applied to
    ...
    def test_login(self):
        ...
        def test_login_thread():
            try:
                ...
                success = validate_naukri_credentials(username, password)
    ...
    def start_applying(self):
        ...
        password = self.password_entry.get().strip()
        if not username or not password:
            messagebox.showwarning("Missing Credentials", "Please enter Naukri login credentials in the Settings tab.")
        ...
        self.job_thread = threading.Thread(
            ...
            args=(search_queries, include_keywords, exclude_keywords, username, password, location, experience, workplace_type),
            daemon=True
        )
        self.job_thread.start()
        
    def run_job_application(self, search_queries, include_keywords, exclude_keywords, username, password, location, experience, workplace_type):
        ...
        try:
            ...
            # Login to Naukri
            self.update_status("Logging in to Naukri...")
            ...
            if not login_success:
                ...
                self.root.after(0, lambda: messagebox.showerror(
                    "Login Failed", 
                    "Could not log in to Naukri. Please check your credentials."
                ))
                driver.quit()
                self.reset_ui()
                return
            ...
            for i, query in enumerate(search_queries):
                ...
                # Use the fetch_jobs_with_requests function with new filters
                jobs, excluded = fetch_jobs_with_requests(driver, query, include_keywords, exclude_keywords, location, experience, workplace_type)
